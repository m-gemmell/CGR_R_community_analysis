...
The workshop will use a 16S dataset that has been pre-analysed with QIIME2 to create the ASV table, taxonomy table, and phylogenetic tree.
...
Throughout this course you will get a lot of practice loading various packages.
...
Then create a new notebook called __"01-Import.ipynb"__ in it. 
...
In our new __cell__ we will use the command `microbiome::readcount()` to store and display the number of reads in each sample within our `phyloseq` object (`pseq`).
...
In a new __cell__ write and run the below code. This will extract the sample data (metadata) to a new object and display the top 6 rows fo this new object.
...
We have a saved `ggplot2` object but now we need to save and display it.
...
Our samples have plateau'd or have gentle slopes towards the end.
...
You can therfore remove the new `phyloseq` object in a new __cell__.
...
alpha_boxplot <- phyloseq::plot_richness(physeq = pseq_rarefy, 
                        ...
                        measures = c("Observed","Chao1","Shannon")) +
                          ...
                          ggforce::geom_sina(alpha=0.5)
#Save ggplot2 object with ggsave
ggsave(filename = "./Alpha_diversity_site_violinplot.png", plot = alpha_boxplot,
       device = "png", dpi = 300, units = "mm", height = 150, width = 250)
#Display plot
...
```{r, fig.align = 'center',out.width= '20%', echo=FALSE }
knitr::include_graphics(path = "figures/difference.png", auto_pdf = TRUE)
``` 
...
## DA: setup
```{r, fig.align = 'center',out.width= '15%', echo=FALSE }
knitr::include_graphics(path = "figures/packages_1.png", auto_pdf = TRUE)
``` 
...
## DA: Preprocess
```{r, fig.align = 'center',out.width= '15%', echo=FALSE }
knitr::include_graphics(path = "figures/water_surface.png", auto_pdf = TRUE)
``` 
...
```{r, fig.align = 'center',out.width= '10%', echo=FALSE }
knitr::include_graphics(path = "figures/portrait.png", auto_pdf = TRUE)
``` 
...
```{r, fig.align = 'center',out.width= '15%', echo=FALSE }
knitr::include_graphics(path = "figures/tiles.png", auto_pdf = TRUE)
``` 
...
It is a good start but the colour gradient is not very useful.
...
## DA: matching bar plot
```{r, fig.align = 'center',out.width= '15%', echo=FALSE }
knitr::include_graphics(path = "figures/bar_chart.png", auto_pdf = TRUE)
``` 

Using code from the [taxa plots chapter](#chaptaxaplots) we'll make a quick class based taxa plot.
...
The beta diversity analysis found significant difference between all the media groupings.
...
## Findings
```{r, fig.align = 'center',out.width= '20%', echo=FALSE }
knitr::include_graphics(path = "figures/compass.png", auto_pdf = TRUE)
``` 
...
```{r, fig.align = 'center',out.width= '15%', echo=FALSE }
knitr::include_graphics(path = "figures/farm.png", auto_pdf = TRUE)
``` 
...
```{r, fig.align = 'center',out.width= '15%', echo=FALSE }
knitr::include_graphics(path = "figures/replicate_difference.png", auto_pdf = TRUE)
```
...
It appears that there is more difference cause by the media than the site.
